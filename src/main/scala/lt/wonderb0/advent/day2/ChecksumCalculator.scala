...
class ChecksumCalculator(spreadsheet: Seq[Seq[Int]]) {
  private case class CalculationResult(checksum: Int)
  def calculate(rowCalculator: (Int, Seq[Int]) => Int = ChecksumCalculator.difference): Int =
    spreadsheet.foldLeft(0)(rowCalculator)
}

object ChecksumCalculator {
  def main(args: Array[String]): Unit = {
    val s = """116	1470	2610	179	2161	2690	831	1824	2361	1050	2201	118	145	2275	2625	2333
          ...
          |571	547	160	237	256	30	496	592	385	576	183	692	192	387	647	233""".stripMargin
    val spreadsheet: Seq[Seq[Int]] = s.lines.map({
      line => line.split("\t").map({num => num.toInt}).toSeq
    }).toSeq

    println(new ChecksumCalculator(spreadsheet).calculate(dividers))
  }

  def difference(currentChecksum: Int, row: Seq[Int]): Int = currentChecksum + (row.max - row.min)
  def dividers(currentChecksum: Int, row: Seq[Int]): Int =
    currentChecksum + row
      .flatMap(divident => row.map(divisor => divident.toDouble / divisor.toDouble))
      .find(divisionResult => divisionResult != 1.0 && divisionResult.floor == divisionResult)
      .getOrElse(0.0)
      .toInt
